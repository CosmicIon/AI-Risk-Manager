...
class CaseManagementService:
    ...
    async def assign(self, case_id: UUID, tenant_id: UUID, user_id: UUID, actor_id: UUID, session):
        ...
        return await self.case_repo.update(session, case_id, tenant_id, CaseUpdate(assigned_to=user_id))
        
    async def update_status(self, case_id: UUID, tenant_id: UUID, new_status: CaseStatus, actor_id: UUID, session, resolution: str | None = None):
        ...
        terminal_states = [CaseStatus.WON, CaseStatus.LOST, CaseStatus.ACCEPTED_LOSS, CaseStatus.EXPIRED]
    ...
    async def search(self, tenant_id: UUID, query: str, filters: dict, page: int, size: int, session):
