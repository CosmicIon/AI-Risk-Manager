...
class GeminiLLMClient:
    ...
    async def _execute_with_retry(self, func, *args, **kwargs):
        ...
        for attempt in range(max_retries + 1):
            try:
                ...
                logger.debug(
                    f"LLM call succeeded in {latency_ms:.2f}ms. Tokens: IN={input_tokens} OUT={output_tokens}"
                )
                return response
    ...
    async def generate_structured(
        self, prompt: str, response_schema: type[T], temperature: float = 0.2
    ) -> T:
        if not self.api_key_configured:
            logger.warning(
                "Using mocked LLM response for generate_structured due to missing API key"
            )
        ...
        try:
            ...
            logger.warning(
                f"Failed to parse JSON on first attempt: {e}. Retrying with correction prompt."
            )
            correction_prompt = f"Your previous response was invalid JSON or mismatched the schema. Error: {str(e)}\n\nOriginal Request:\n{prompt}"

            response = await self._execute_with_retry(
                model.generate_content, correction_prompt, generation_config=config
            )
            try:
                return response_schema.model_validate_json(response.text)
            except Exception as e2:
                raise LLMResponseError(
                    f"Failed to parse structured LLM response after retry: {e2}"
                ) from e2
    ...
    async def generate_text(
        ...
        response = await self._execute_with_retry(
            self.model.generate_content, prompt, generation_config=config
        )
