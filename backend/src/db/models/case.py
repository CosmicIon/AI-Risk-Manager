...
class Case(Base):
    ...
    assigned_to: Mapped[uuid.UUID | None] = mapped_column(ForeignKey("risk_users.id", ondelete="SET NULL"), nullable=True)
    ...
    __table_args__ = (
        ...
        Index("idx_cases_open", "tenant_id", "deadline", postgresql_where=sa_text("status NOT IN ('WON', 'LOST', 'EXPIRED', 'ACCEPTED_LOSS')")),
    )
