...
> Every `- [ ]` item is actionable. Mark `- [/]` when in progress, `- [x]` when complete.
...
- [ ] `make up` starts all 7 infrastructure containers with passing health checks
- [ ] `curl http://localhost:8000/health` returns `{"status": "ok"}`
- [ ] `curl http://localhost:8000/readiness` returns status for each dependency
- [ ] `make test` runs (even if no tests yet) without import errors
...
- [ ] All Pydantic models raise `ValidationError` with invalid data (e.g., `return_amount > order_amount`, `risk_score` outside 0-100, missing required fields)
- [ ] `ChargebackNotification.deadline` auto-computed correctly: Visa → +30 days, Mastercard → +45 days
- [ ] `ChargebackNotification.idempotency_key` is deterministic for the same network + ARN
- [ ] All enums are JSON-serializable (string values)
- [ ] Avro schemas are valid and parseable by `fastavro`
- [ ] `CostWeightedMetrics.cost_weighted_loss` computation is verified with known inputs
...
- [ ] **2.1 — Async session factory (`backend/src/db/session.py`)**
  - [ ] Create `async_engine` using `create_async_engine(settings.DATABASE_URL, pool_size=20, max_overflow=10, pool_pre_ping=True)`
  - [ ] Create `AsyncSessionLocal` using `async_sessionmaker(async_engine, expire_on_commit=False)`
  - [ ] Implement `async def get_db_session() -> AsyncGenerator[AsyncSession, None]` as a FastAPI dependency
  - [ ] Implement `async def get_db_session_with_tenant(tenant_id: UUID)` that sets `SET app.current_tenant = '{tenant_id}'` for RLS

- [ ] **2.2 — ORM models**
  - [ ] `Tenant` (`backend/src/db/models/tenant.py`): `id: UUID (PK)`, `name: str`, `api_key_hash: str`, `fp_cost_per_unit: Decimal = 500`, `fn_cost_per_unit: Decimal = 2000`, `policy_config: JSON` (stores `PolicyConfig` as JSONB), `created_at: DateTime`, `is_active: bool = True`
  - [ ] `User` (`backend/src/db/models/user.py`): `id: UUID (PK)`, `tenant_id: UUID (FK -> Tenant)`, `email: str (unique)`, `password_hash: str`, `role: str` (enum: `admin`, `analyst`, `ml_engineer`, `compliance`), `created_at: DateTime`, `is_active: bool = True`
  ...
  - [ ] `EvaluationRun` (`backend/src/db/models/evaluation_run.py`): `id: UUID (PK)`, `model_name: str`, `model_version: str`, `holdout_set_version: str`, `holdout_set_hash: str`, `metrics: JSON` (stores `CostWeightedMetrics`), `threshold: Float`, `is_champion: bool = False`, `report_url: str`, `evaluated_at: DateTime`
  - [ ] `AuditLog` (`backend/src/db/models/audit_log.py`): `id: UUID (PK)`, `tenant_id: UUID (FK -> Tenant)`, `case_id: UUID (FK -> Case, nullable)`, `actor_id: UUID (FK -> User)`, `action: str`, `old_value: JSON (nullable)`, `new_value: JSON (nullable)`, `timestamp: DateTime (server_default=func.now())`, `ip_address: str (nullable)`. Partitioned by month on `timestamp` for retention compliance

- [ ] **2.3 — Alembic migration setup**
  - [ ] Create `backend/alembic/alembic.ini` pointing to `backend/alembic/`
  - [ ] Create `backend/alembic/env.py` with async engine support, importing all ORM models for autogenerate
...
- [ ] **2.4 — Repository layer**
  - [ ] `CaseRepository` (`backend/src/db/repositories/case_repo.py`):
  ...
  - [ ] `ChargebackRepository` (`backend/src/db/repositories/chargeback_repo.py`):
  ...
  - [ ] `EvaluationRepository` (`backend/src/db/repositories/evaluation_repo.py`):
...
- [ ] **2.5 — Database seeding script (`backend/scripts/seed_db.py`)**
  - [ ] Create a test tenant with `id = UUID("00000000-0000-0000-0000-000000000001")`
  - [ ] Create 3 test users (admin, analyst, ml_engineer) for the test tenant
  - [ ] Create 10 sample cases across all 4 case sources with varying statuses
  - [ ] Create 5 sample chargeback records with evidence bundles

### Acceptance Criteria

- [ ] `alembic upgrade head` runs without errors on a clean database
- [ ] `alembic downgrade base` fully reverses all migrations
- [ ] RLS policy prevents cross-tenant data access when `app.current_tenant` is set
- [ ] `CaseRepository.create` raises `DuplicateIngestionError` on duplicate `(tenant_id, source, source_id)`
- [ ] `CaseRepository.update` writes an audit log entry on every status change
- [ ] `ChargebackRepository.get_win_rate` returns correct statistics
- [ ] Seeding script creates all expected records
...
- [ ] **3.1 — Redis client (`backend/src/integrations/redis_client.py`)**
  - [ ] `class RedisClient`: wraps `redis.asyncio.Redis` connection pool
  - [ ] `async def get_feature_vector(customer_id: str, tenant_id: UUID) -> dict[str, float] | None`: fetch from key `features:{tenant_id}:{customer_id}`, deserialize JSON
  - [ ] `async def set_feature_vector(customer_id: str, tenant_id: UUID, features: dict, ttl: int = 3600)`
  - [ ] `async def increment_counter(key: str, window_seconds: int) -> int`: for rate limiting, uses `INCR` + `EXPIRE`
  - [ ] `async def check_rate_limit(identifier: str, limit: int, window: int) -> tuple[bool, int]`: returns (is_allowed, remaining)
  - [ ] `async def health_check() -> bool`: `PING`
  - [ ] Connection pool config: `max_connections=50`, `decode_responses=True`, `socket_timeout=2.0`, `retry_on_timeout=True`

- [ ] **3.2 — Kafka producer (`backend/src/integrations/kafka_producer.py`)**
  - [ ] `class TypedKafkaProducer`: wraps `aiokafka.AIOKafkaProducer`
  - [ ] `async def send_event(topic: str, event: BaseModel, key: str | None = None)`: serialize event to JSON bytes, send with optional partition key. Validate event type against `TOPIC_MAP` from `core/events.py` — raise `SchemaValidationError` if event type doesn't match topic
  - [ ] `async def send_batch(topic: str, events: list[BaseModel], key_fn: Callable)`: batch send with per-event keys
  - [ ] Retry config: `retries=3`, `retry_backoff_ms=100`, `acks="all"` (exactly-once semantics)
  - [ ] `async def health_check() -> bool`: produce to `__health_check` topic and verify

- [ ] **3.3 — Qdrant client (`backend/src/integrations/qdrant_client.py`)**
  - [ ] `class QdrantVectorStore`: wraps `qdrant_client.AsyncQdrantClient`
  - [ ] `COLLECTION_NAME = "chargeback_cases"`
  - [ ] `async def ensure_collection()`: create collection if not exists with `vectors_config=VectorParams(size=1536, distance=Distance.COSINE)`, payload schema indexes on `reason_code`, `network`, `amount_bucket`, `outcome`
  - [ ] `async def upsert_case(case_id: str, embedding: list[float], payload: dict)`: upsert a chargeback case with metadata payload
  - [ ] `async def search_similar(query_embedding: list[float], reason_code: str | None, network: str | None, limit: int = 5) -> list[dict]`: ANN search with optional payload filters. Returns `[{"case_id": str, "score": float, "outcome": str, "narrative_summary": str}]`
  - [ ] `async def health_check() -> bool`: list collections

- [ ] **3.4 — LLM client (`backend/src/integrations/llm_client.py`)**
  - [ ] `class GeminiLLMClient`: wraps `google.generativeai` (Gemini 2.5 Flash)
  - [ ] `async def generate_structured(prompt: str, response_schema: type[BaseModel], temperature: float = 0.2) -> BaseModel`: call Gemini with JSON mode, parse response into the given Pydantic model. On parse failure: retry once with a correction prompt. On second failure: raise `LLMResponseError`
  - [ ] `async def generate_text(prompt: str, temperature: float = 0.3, max_tokens: int = 4096) -> str`: plain text generation
  - [ ] Implement retry decorator: exponential backoff on HTTP 429/503, max 3 retries, jitter ±500ms
  - [ ] Implement token tracking: log `input_tokens`, `output_tokens`, `latency_ms` per call
  - [ ] `async def health_check() -> bool`: send trivial prompt, verify response

- [ ] **3.5 — Langfuse client (`backend/src/integrations/langfuse_client.py`)**
  - [ ] `class LangfuseTracer`: wraps `langfuse.Langfuse`
  - [ ] `def create_trace(case_id: str, name: str) -> Trace`: create a new trace for a chargeback case
  - [ ] `def create_span(trace: Trace, name: str, input: dict, output: dict | None = None) -> Span`: create a span within a trace (for each agent step)
  - [ ] `def create_generation(trace: Trace, name: str, prompt: str, completion: str, model: str, tokens: dict)`: log LLM generation
  - [ ] `def score_trace(trace: Trace, name: str, value: float, comment: str | None = None)`: score a trace (e.g., win probability, evidence completeness)

- [ ] **3.6 — MinIO/S3 client (`backend/src/integrations/minio_client.py`)**
  - [ ] `class ObjectStoreClient`: wraps `boto3` S3 client configured for MinIO
  - [ ] `async def upload_file(bucket: str, key: str, data: bytes, content_type: str) -> str`: returns full URL
  - [ ] `async def download_file(bucket: str, key: str) -> bytes`
  - [ ] `async def upload_model_artifact(model_name: str, version: str, model_bytes: bytes, metadata: dict) -> str`: uploads to `models/{model_name}/{version}/model.onnx` + `metadata.json`
  - [ ] `async def download_holdout_set(version: str) -> bytes`: downloads from `holdout/{version}/data.parquet`
  - [ ] `async def ensure_buckets()`: create `models`, `holdout`, `evidence`, `reports` buckets if they don't exist

- [ ] **3.7 — Dependency injection (`backend/src/api/deps.py`)**
  - [ ] `async def get_redis() -> RedisClient`: singleton pattern via `app.state`
  - [ ] `async def get_kafka_producer() -> TypedKafkaProducer`: singleton via `app.state`
  - [ ] `async def get_qdrant() -> QdrantVectorStore`: singleton via `app.state`
  - [ ] `async def get_llm() -> GeminiLLMClient`: singleton via `app.state`
  - [ ] `async def get_langfuse() -> LangfuseTracer`: singleton via `app.state`
  - [ ] `async def get_object_store() -> ObjectStoreClient`: singleton via `app.state`
  - [ ] `async def get_current_user(token: str = Depends(oauth2_scheme)) -> User`: JWT decode + DB lookup
  - [ ] `async def get_current_tenant(user: User = Depends(get_current_user)) -> Tenant`: extract tenant from user
  - [ ] Wire up `lifespan` handler in `main.py`: initialize all clients on startup, close connections on shutdown

### Acceptance Criteria

- [ ] All clients connect to their respective Docker containers
- [ ] All `health_check()` methods return `True` when services are up, `False` when down
- [ ] `TypedKafkaProducer.send_event` rejects events that don't match `TOPIC_MAP`
- [ ] `GeminiLLMClient.generate_structured` returns a valid Pydantic model
- [ ] `RedisClient.check_rate_limit` correctly enforces limits
- [ ] `QdrantVectorStore.search_similar` returns results with correct payload filtering
...
- [ ] **4.1 — Synthetic data generator (`backend/scripts/generate_synthetic_data.py`)**
  - [ ] Generate 100,000 synthetic transactions with realistic distributions: amounts (lognormal, mean ₹2,500), timestamps (non-uniform, more activity 10am-10pm IST), categories (weighted: electronics 20%, fashion 30%, groceries 25%, other 25%)
  - [ ] Generate 5,000 return requests (5% abuse rate) with labels: `is_abusive: bool`. Abuse patterns: high frequency (>3 returns/month), high value concentration, new accounts (<30 days), mismatched device fingerprints
  - [ ] Generate 500 chargeback cases with outcomes (40% won, 50% lost, 10% pending) across reason codes
  - [ ] Output as Parquet files to `data/synthetic/transactions.parquet`, `data/synthetic/returns.parquet`, `data/synthetic/chargebacks.parquet`
  - [ ] Create time-based train/test split: 80% train (earlier dates), 20% holdout (later dates) in `data/synthetic/holdout/`

- [ ] **4.2 — Return-risk feature engineering (`backend/src/ml/models/return_risk/features.py`)**
  - [ ] `def compute_features(customer_id: str, order: dict, history: list[dict]) -> dict[str, float]`: computes all 50 features:
  ...
  - [ ] `def compute_features_from_redis(redis_client, customer_id: str, order: dict) -> FeatureVector`: fetches pre-computed features from Redis, computes missing ones on-the-fly, returns `FeatureVector` with `is_degraded=True` if any feature source was unavailable

- [ ] **4.3 — Return-risk model config (`backend/src/ml/models/return_risk/config.py`)**
  - [ ] `FEATURE_NAMES: list[str]` — ordered list of all 50 feature names
  - [ ] `CATEGORICAL_FEATURES: list[str]` — `["high_risk_category_flag", "is_new_account", "is_new_device", "is_weekend_return"]`
  - [ ] `HYPERPARAMETERS: dict` — `{"objective": "binary", "metric": "binary_logloss", "num_leaves": 63, "learning_rate": 0.05, "n_estimators": 500, "scale_pos_weight": 19.0, "min_child_samples": 20, "subsample": 0.8, "colsample_bytree": 0.8, "reg_alpha": 0.1, "reg_lambda": 1.0, "max_depth": -1, "verbose": -1}`
  - [ ] `CLASSIFICATION_THRESHOLD: float = 0.35` (tuned for cost-weighted optimization, not F1)

- [ ] **4.4 — Return-risk model training (`backend/src/ml/models/return_risk/train.py`)**
  - [ ] `def train(train_path: str, config: dict) -> lgb.Booster`: load Parquet, extract features using `features.py`, train LightGBM with `config.HYPERPARAMETERS`, return trained booster
  - [ ] `def export_to_onnx(booster: lgb.Booster, output_path: str)`: convert LightGBM booster to ONNX using `onnxmltools`, save to `output_path`
  - [ ] `def save_metadata(output_dir: str, feature_names: list, hyperparams: dict, train_metrics: dict)`: write `metadata.json` with model version, training timestamp, feature list, hyperparameters, training set metrics
  - [ ] CLI entrypoint: `python -m src.ml.models.return_risk.train --data data/synthetic/returns.parquet --output models/return_risk/v1/`

- [ ] **4.5 — Chargeback win-probability model (`backend/src/ml/models/chargeback_win/`)**
  - [ ] `features.py`: `compute_features(chargeback: dict, evidence: dict) -> dict[str, float]`: 20 features including `evidence_completeness_score`, `reason_code_historical_win_rate`, `amount_bucket`, `days_to_deadline`, `has_delivery_proof`, `has_3ds_log`, `has_avs_match`, `has_customer_communication`, `similar_case_best_outcome`, `merchant_historical_win_rate`, `network_factor`, `evidence_item_count`, `narrative_length`, `transaction_age_days`
  - [ ] `train.py`: same pattern as return-risk, using LightGBM with `objective="binary"`, output probability

- [ ] **4.6 — Fraud-spike detector models (`backend/src/ml/models/fraud_spike/`)**
  - [ ] `isolation_forest.py`: `class IsolationForestDetector` — train on normal transaction features (amount, velocity, geo spread), `detect(features: np.ndarray) -> tuple[bool, float]` returns (is_anomaly, anomaly_score). Export to ONNX
  - [ ] `lstm_autoencoder.py`: `class LSTMAutoencoder(nn.Module)` — PyTorch LSTM autoencoder trained on transaction sequences (window of 60 seconds). `detect(sequence: np.ndarray) -> tuple[bool, float]` based on reconstruction error threshold. Export to ONNX via `torch.onnx.export`
  - [ ] `ensemble.py`: `class FraudSpikeEnsemble` — combines Isolation Forest + LSTM autoencoder. `detect(point_features: np.ndarray, sequence_features: np.ndarray) -> AnomalyAlert`: requires both models to agree for `CRITICAL`, either one for `WARNING`, neither for `INFO`. Implements calendar-aware threshold adjustment (`threshold_multiplier: float` during registered events)

- [ ] **4.7 — ONNX Runtime serving (`backend/src/ml/serving/onnx_runtime.py`)**
...
- [ ] **4.8 — Model registry (`backend/src/ml/serving/model_registry.py`)**
  - [ ] `class ModelRegistry`:
...
- [ ] **4.9 — SHAP explainability (`backend/src/ml/explainability/shap_explainer.py`)**
  - [ ] `class SHAPExplainer`:
  ...
  - [ ] `class ExplanationFormatter` (`backend/src/ml/explainability/formatter.py`):
...
- [ ] Synthetic data generator produces correctly shaped Parquet files with realistic distributions
- [ ] Feature engineering produces exactly 50 features with correct names and types
- [ ] LightGBM model trains on synthetic data and produces a valid `.onnx` file
- [ ] ONNX Runtime loads the model and produces predictions with P99 latency < 5ms
- [ ] Model registry correctly manages champion/shadow models
- [ ] SHAP explainer produces top-5 features with correct directionality
- [ ] Fraud spike ensemble correctly applies calendar-aware thresholds
...
- [ ] **5.1 — Metrics computation (`backend/src/ml/evaluation/metrics.py`)**
  - [ ] `def compute_binary_metrics(y_true: np.ndarray, y_prob: np.ndarray, threshold: float) -> CostWeightedMetrics`:
  ...
  - [ ] `def find_optimal_threshold(y_true: np.ndarray, y_prob: np.ndarray, fp_cost: float, fn_cost: float, thresholds: np.ndarray = np.arange(0.05, 0.95, 0.01)) -> tuple[float, CostWeightedMetrics]`: sweep thresholds, return the one minimizing cost-weighted loss
  - [ ] `def compute_calibration_curve(y_true: np.ndarray, y_prob: np.ndarray, n_bins: int = 10) -> dict`: returns `{"bin_means": [...], "fraction_positive": [...], "ece": float}` (Expected Calibration Error)
  - [ ] `def compute_threshold_curve(y_true: np.ndarray, y_prob: np.ndarray, fp_cost: float, fn_cost: float) -> list[dict]`: returns `[{"threshold": float, "precision": float, "recall": float, "cost_weighted_loss": float}, ...]` for dashboard visualization

- [ ] **5.2 — Holdout set manager (`backend/src/ml/evaluation/holdout_manager.py`)**
...
- [ ] **5.3 — Evaluation harness (`backend/src/ml/evaluation/harness.py`)**
...
- [ ] **5.4 — Drift detector (`backend/src/ml/evaluation/drift_detector.py`)**
  - [ ] `def compute_psi(reference: np.ndarray, current: np.ndarray, n_bins: int = 10) -> float`: Population Stability Index
  - [ ] `def compute_kl_divergence(reference: np.ndarray, current: np.ndarray, n_bins: int = 10) -> float`: KL-divergence with smoothing (add epsilon to avoid log(0))
  - [ ] `def detect_drift(reference_data: pd.DataFrame, current_data: pd.DataFrame, feature_names: list[str]) -> list[DriftReport]`: compute PSI + KL for each feature, return list of `DriftReport` with `is_drifted` and `requires_retrain` flags
  - [ ] `def compute_prediction_drift(reference_preds: np.ndarray, current_preds: np.ndarray) -> dict`: compare prediction distributions to detect model-level drift (not just feature-level)

- [ ] **5.5 — Evaluation CLI (`backend/scripts/run_evaluation.py`)**
  - [ ] Argparse CLI: `python scripts/run_evaluation.py --model return_risk --version v1 --holdout v1 --fp-cost 500 --fn-cost 2000`
  - [ ] Prints metrics table to stdout
  - [ ] Outputs full report to S3
  - [ ] Exits with code 0 if gate passes, code 1 if fails (for CI integration)

- [ ] **5.6 — CI model evaluation gate (`.github/workflows/model-evaluation.yml`)**
  - [ ] Trigger on changes to `backend/src/ml/models/**`, `models/**`, `data/holdout/**`
  - [ ] Run `scripts/run_evaluation.py` for each modified model
  - [ ] Fail the pipeline if any model fails the gate check
